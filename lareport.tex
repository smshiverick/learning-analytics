...
select the regressors that best predict the target outcome, with non-relevant 
variables excluded from the final model. In this sense, the t-value provides 
a measure of the importance of a given predictor. With random forests, node 
purity measures branch homogeneity for classification tasks, whereas MSE 
reduction is used for variable selection on regression tasks. Variable 
importance with random forests is affected by the number of categories
...
their original scales for meaningful interpretation of the results. Furthermore, 
it may not be feasible to assess the broader construct of learning using a 
single measure of performance such as a final course grade. A thorough 
comparison of predictive models could include multiple dependent measures 
