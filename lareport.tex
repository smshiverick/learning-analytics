...
provided a more parsimonious solution that is more interpretable, given that 
the predicted value of student performance is based on the linear combination 
of all the independent variables and observed values in the model. Linear 
regression performs well when working with standard data in which the number 
of observation is larger than the number of features ($\textit{n}>>\textit{p}$); 
ridge regression and the lasso are preferred models when working with a dataset 
has a very large number of predictors ($\textit{p}>>\textit{n}$).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The simple regression tree model selected course subject as the root node in 
the training set, with mother's education, weekly study time, absences, mother's 
job, and student age as the most informative variables for predicting 
performance. In the testing set, the algorithm selected plans for higher 
education as the root node, with mother's education, residence area (rural 
versus urban), course subject, and absences selected as the most informative 
features. Random forests regression corrects for overfitting in a single 
regression tree by constructing many trees and averaging across the 
