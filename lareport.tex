...
The results revealed that the supervised learning models identified different 
sets of features as important for predicting student performance. The general 
linear model selected thirteen predictors variables in the training set, of
which, only six significantly predicted student performance in the test set. 
Plans for higher education, mother's education level, and weekly study time 
were positively correlated with performance, whereas course subject, school 
support, and romantic relationships were negatively associated with performance. 
Ridge regression reduces error by shrinking the coefficient estimates towards 
zero, but includes the full set of 27 predictor variables, with a tradeoff of
slightly higher bias and risk of underfitting the relationships between the 
features and target outcome. The lasso model lasso performed variable 
selection by forcing 7 of the non-relevant coefficient estimates to zero. 
This produced a simpler model based on subset of 20 features; however, the
final general linear model with 13 independent variables was more interpretable
than the lasso model with 20 variables. 




over ridge regression as it 
