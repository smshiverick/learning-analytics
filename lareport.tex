...
\begin{table*}
  ...
  \begin{tabular}{lllll}
    ...
    \midrule
    Mother's Education  &  1.368 &   &      Absences	        &  16.042	\\
    Absences            &  0.884 &   &      Course	            &  11.669   \\
    Area (Urban/Rural)  &  0.592 &   &      Mother's Education  &   7.749	\\
    Higher Education    &  0.575 &   &      Age	                &   7.488   \\   
    Course              &  0.481 &   &      Go Out Friends	    &   6.560   \\ 
    Weekly Alcohol      &  0.473 &   &      Study Time          &   4.506   \\
    Mother's Job        &  0.463 &   &      Health	            &   3.875   \\  
    Father's Education  &  0.399 &   &      Weekly Alcohol 	    &   3.549	\\      
    Go Out Friends      &  0.389 &   &      Family Relations 	&   3.446	\\
    Daily Alcohol       &  0.373 &   &      Free Time	        &   3.330	\\
    Age                 &  0.370 &   &      Father's Job        &   2.839   \\
    School Support      &  0.347 &   &      Higher Education    &   2.478   \\  
    Study Time          &  0.321 &   &      School Support  	&   2.472   \\
    Sex                 &  0.318 &   &      Extra Activities	&   2.428   \\
    Father's Job        &  0.278 &   &      Daily Alcohol 	    &   2.278   \\     
    Free Time           &  0.272 &   &      Mother's Job        &   2.123	\\
    Internet Access     &  0.204 &   &      Family Size         &   2.104   \\   
    Family Relations    &  0.181 &   &      Father's Education 	&   2.065   \\
    Extra Activities    &  0.179 &   &      Guardian	        &   2.038   \\
    Travel Time         &  0.122 &   &      Sex	                &   1.824   \\ 
    Guardian            &  0.121 &   &      Romantic Relation	&   1.780	\\
    Family Size         &  0.112 &   &      Internet Access	    &   1.760	\\
    Family Support      &  0.103 &   &      Travel Time	        &   1.574   \\
    Health              &  0.097 &   &      Paid Extra Courses	&   1.286	\\ 
    Romantic Relation   &  0.093 &   &      Family Support    	&   1.221   \\
    Paid Extra Courses  &  0.075 &   &      Area (Urban/Rural)	&   1.218	\\
    Parents' Relation   &  0.056 &   &      Parents' Relation   &   0.301   \\
    \bottomrule
  \end{tabular}
\end{table*}
...
The mean squared error (MSE) for the gradient boosted regression tress  was 
19.687, which indicates this model did not perform better than random forests. 
Feature importance for the gradient boosted regression trees is presented on 
the right side of Table 6. Absences and course subject were selected as the
two most important features for predicting student performance. The algorithm
selected mother's education level, student age, going out with friends, and
weekly study time as the next most informative variables, in descending order 
of importance. Plans for higher education was not selected among the most 
important variables for predicting student performance, In contrast to the 
single regression tree and random forest model. The predicted value of 
student performance is determined by the linear combination or weighted
average of all the predictor values in the model.  

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{DISCUSSION}

The results revealed that student performance was better in Portugese than 
the mathematics course and course subject was a significant predictor across 
all of the models. The supervised learning models differed, however, in the 
sets of features they identified as important for predicting performance. 
The general linear model selected thirteen predictors variables in the training
set, of which, only six variables were significant predictors of student 
performance in the test set. Plans for higher education, mother's education 
level, and weekly study time were positively correlated with performance, 
whereas course subject, school support, and romantic relationships were 
negatively associated with performance. Ridge regression reduced model error 
by shrinking the coefficient estimates towards (not exactly) zero, but 
included the full set of 27 predictor variables, with a tradeoff of slightly 
higher bias and risk of underfitting the relationships between the features 
and the target outcome. The lasso model lasso performed variable selection by 
forcing 7 non-relevant coefficient estimates to exactly zero which produced a 
simpler model based on a subset of 20 features. The features with the largest 
coefficients were course subject, plans for higher education, school support, 
home internet access, and romantic relationships. The coefficient estimates for 
mother's level of education and weekly study time were smaller in the ridge and 
lasso models than the general linear model. Although the lasso model with 20 
features produced a more interpretable solution than the ridge regression, 
the solution from the final general linear model with 13 independent variables
was more interpretable than the lasso model with 20 variables. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The simple regression tree models wwre prepruned to a depth of four, as 
unpruned trees can be very complex and difficult to interpret. In the training 
set, the algorithm selected course subject, mother's education level, weekly 
study time, absences, mother's job, and student age, as the most informative 
variables for predicting student performance. In the testing set, plans for 
higher education was the root node, mother's education, rural or urban area, 
course subject, and absences were selected as the most informative features. 
A single decision tree is highly prone to overfitting the training data, and 
the random forests regression corrects for this by constructing many trees 
and averaging the predicted values across tress. 

are preferred for modeling non-linear relationship and 
if unpruned can result in very complex models .
...
in 
data student performance the independent variables is
non-linearare preferred if student performance is non-linear. 
