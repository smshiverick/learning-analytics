...
Predictive modeling was used to detect patterns in education data and identify 
factors that contribute to student learning and performance. This paper 
compared six models of student performance (linear regression, ridge regression, 
lasso, regression trees, random forests regression, gradient boosting) and used 
feature importance to identify which variables were important for predicting 
performance. The sample dataset consisted of N=1044 observations from two 
secondary schools in Portugal \cite{cortez08}. Performance was assessed by 
final grades (range; 0-20) in two courses, mathematics and Portugese. The models 
were fit to a subset of training data and evaluated on a hold out testing set. 
The models all selected a similar set of variables as important for predicting 
student performance; the models differed in the number, weighting, order and 
importance they gave to the predictor variables. Feature importance was used
for variable selction. Course subject was selected as an important predictor of 
performance in all of the models; students in the Portugese course performed 
better than students in mathematics. Linear regression selected thirteen 
predictors of student performance in the training set, of which, six were 
significant in the test set. Plans for higher education, mother's education 
level, and weekly study time were positively correlated with student performance, 
whereas course subject, school support, and romantic relations were negatively 
associated with performance. The regression trees model selected students' plans 
for higher education, mother's education, residence area (rural/urban), course 
subject, and student absences as key nodes. Random forests regression selected 
mother's education as the most informative variable for predicting performance, 
whereas the gradient boosted regression tree selected student absences as the 
most important feature for predicting student performance. There was a tradeoff 
between model complexity and interpretability. Simple linear regression and a 
single decision tree were easier to interpret than more complex models. The 
...
Predictive modeling provides a set of analytic tools for detecting patterns in 
education data, selecting important features for predicting student success 
and identifying students potentially at risk. This study compared several 
supervised learning models with sample data to determine which approach is 
best for modeling student performance and success. 
...
result of a single statistical model, this work suggests that comparing the
results of several models is useful for selecting the best model given the
question at hand and the available data. Educational data mining (EDM) and 
learning analytics (LA) reserchers are increasingly focused on outcomes from
learning in online platforms, and in future it will become important to 
...
reviewed EDM and LA findings and 
