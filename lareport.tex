...
and the environments in which it occurs \cite{siemens13}. Predictive modeling 
provides useful methods for analyzing the factors that contribute to student 
success and identifying individuals at risk for failure or dropping out. 
This study reviewed EDM and LA findings and compared several supervised 
learning models with sample data to determine which approach is best for 
modeling student performance and success. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Educational Data Mining (EDM) and Learning Analytics (LA)}
...
\subsubsection{Student / Student Behavior Modeling}.
...
Previous studies have used classification methods such as logistic regression, 
naive Bayes models, decision trees, and neural networks to predict student 
performance as a binary outcome \cite{Lykourentzou09}. Empirical findings 
indicate that, in addition to course assessments such as the number of quizzes 
successfully passed, engagement and participation in course activities is the 
most influential predictor of final grades \cite{Papamitsiou14, romerozaldivar12}. 
Furthermore, a student's sense of belonging in a course is essential for 
engagement and improved satisfaction, which in turn, can reduce student dropout.
...
The two main branches of statistical learning or machine learning are 
supervised learning and unsupervised learning. Supervised learning problems 
involve prediction about a specific target variable or outcome of interest. 
If a dataset has no target outcome, unsupervised learning methods 
(i.e., clustering) can be used to discover structure in unlabeled data. 
The current project uses supervised learning models to predict student 
performance based on final course grade. The model is trained` on a set of 
input-output pairs and then used to predict testing data that has been set
aside. The two major approaches to supervised learning are regression and 
classification. When the target variable to be predicted is continuous, or 
there is continuity between the outcome (e.g., home values, income), a 
regression model is used to test the set of features that predict the target 
variable. If the target is a class label, or set of categorical or binary 
outcomes, classification is used to predict which class or category label 
new instances will be assigned to. Comparing the performance of different 
learning algorithms can help determine which model is best for a given 
problem and the data available \cite{raschka17}. Several different 
...
(X[1] ... X[p]), with some measure of error. The predicted value of the target 
outcome can be thought of as the weighted sum of the input features with the 
weights or coefficients (i.e., beta values) indicating the influence of a given 
feature on the outcome. If the number of observations ('n') is much larger than 
the number of features ('p'), ordinary least squares estimates will have low 
variance, and perform well on test observations. If n is not much larger than p,
high variability in the least squares fit can result in overfitting and poor 
...
better prediction accuracy and model interpretability \cite{statlearn13}. 
...
In many cases, multiple features in a linear regression are not associated 
with the target response. Three methods for improving the fit of linear models 
are (a) subset selection, (b) dimension reduction, and (c) regularization or 
shrinkage. This paper focuses on regularization models that include all 
\textit{p} predictor features, but constrain or \textit{regularize} the 
coefficient estimates by shrinking them towards zero. Regularization reduces 
variablity, which in turn improves test set accuracy, with a slight increase in 
bias. Shrinking the coefficient estimates of irrelevant features toward zero 
...
interpretable models based on a subset of features. The lasso uses an L1 
penalty which has the effect of forcing some of the coefficient estimates to be
equal to exactly zero when the tuning parameter lambda is sufficiently large 
...
overfit data used to train the model. A common strategy to prevent overfitting 
is pre-pruning which limits the maximum depth of the tree, or number of leaves. 
...
In addition to the number of trees and pre-pruning, an important parameter for 
gradient boosting is the \emph{learning rate} which determines how strongly 
...
trees to the ensemble also increases model complexity. Gradient boosting and 
random forests perform well on similar tasks and data. A common practice is 
to first construct random forests and then use gradient boosting to improve 
model accuracy \cite{muller17}. 
...
This study applies supervised learning models to identify the set of 
demographic characteristics that predict student performance and success. 
...
A good strategy for building a regression tree is to
...
considered at each node to determine the randomness of each tree. 

In general, random forests work well without very much parameter tuning or 
scaling of data. The important parameters for the random forests algorithm are the number of 
...
from each other, and reduces overfitting.


A large number of trees is generated and the results are averaged. 
...
Adding more trees to the ensemble also increases model complexity. 
