...
using the rpart() function in R, with all 27 independent variables. Cross-
validation was used to determine the optimal tree complexity. The decision tree
was constructed using recursive binary splitting to grow a large tree on the 
training data. K-fold cross-validation was used to choose the best 

alpha, dividing the training 
observations into k-folds, and for each k=1...K, 

model was prepruned to a maximum depth of 3, which means the algorithm split on 
three consecutive features. The decision tree 

(2) A cost complexity (i.e., weakest link) pruning is applied
to the large tee to obtain a sequence of best subtrees, as a function of alpha. 
(3) (a) Steps 1 and 2 were 
...
Figure 4 shows the resulting regression tree had 7 terminal nodes with course 
subject as the root node. Course subject was a dummy variable and the branch to 
the left represents students in the mathematics course (39\%); the branch to 
the right represents students in the Portugese course (61\%). In addition to 
course subject, the algorithm split at nodes for mother's education level, 
weekly study time. student absences, and age in constructing the tree. 
The predicted mean value of student performance at the root node, and 
the predicted values ranged from 5 for students with one or more absences 
who were 18 years or older (4\%), to 13 for students whose mother's had 
attained some level of higher education (18\%). 
...
The regression tree model was evaluated on the test set with a maximum depth
of 3, which yielded a tree with 6 terminal nodes (shown in Figure 5). 
The MSE for the regression tree on the testing set was 17.608. As seen in 
Figure 5, the root node was students' plans for higher education. In 
addition to higher education, mother's education level, area (urban / rural),
course subject, and student absences were used to construct the tree.
Following the right branch from the root node, the 91\% of students in 
the training set with plans for higher education had a predicted value 
of student performance of 12, whereas on the left branch for students with 
no plans for higher education (9\%) predicted mean performance was only 7.  

For students with plans for higher education, the next split was on mother's
education level. The branch to the right represented 74\% of the testing set
whose mothers had an education level of 5th grade or higher, where the 
predicted performance was 12. Following this branch to the right, the mean
predicted student performance for 55\% of students in urban areas was 13, 
whereas the mean predicted performance for the 19\% of students in rural 
areas was 11. For the 17\$ of students whose mother's had secondary education 
or less, the mean predicted performance was 10; this branch next split on 
course topic, and the mean predicted performance for students in the 
Portugese course (12\%) was 11, and the mean performance for students in 
the mathematics course (5\%) was only 8.2. For students with no plans for 
higher education (9\%), the tree split at the next node on absences, 
where the mean predicted performance was 7. The mean predicted performance
for students with no absences (5\%) was 9.1, whereas for students in the 
testing set with one or more absences, mean performance was 4.6. 
...
The  random forests regression model was fit to the training set using
the randomforest() model in R, with 500 trees, and three variables considered 
at each split (e.g., mtry=3).


Random forests build many different decision trees, and then averages the trees 
to reduce variance. The advantage of randome forests is that it provides a more accurate model, but the tradeoff is that the large ensemble of trees can be more difficult to interpret. 

random forest package was loaded into R, and the
