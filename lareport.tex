...
which indicates better performance than for a single decision tree. The random 
forests (RF) algorithm provides feature importance as a model summary; for 
regression this is measured in terms of percent increase in MSE. The left side 
of Table 6 provides the feature importance for the RF regression sorted by 
percent increase in MSE. The algorithm selected mother's education as the most 
informative feature for predicting student performance (final grade). In 
contrast to the single decision tree, number of absences and area (urban/rural)
were selected among the most important features in the model. Similar to the 
single tree, plans for higher education and course subject were among the most
