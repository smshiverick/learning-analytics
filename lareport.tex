...
and non-significant regressors are excluded from the final model. The t-value 
provides a general measure of the effect of a given predictor. With random 
forests, node purity is a measure of branch homogeneity for classification 
tasks, whereas MSE reduction is used for variable selection for regression. 
Variable importance with random forests regression is affected by the number 
of categories and scale of measurement of the predictor variables 
...
are measured on different scales or vary in number of categories. Much of the 
student data collected by educational institutions is measured on different 
scales. One approach for normalizing or regularizing data is to transform 
variables measured on different scales to the same scale, with a mean of zero
and standard deviation of one. Future studies should examine variable importance 
for datasets where variables have been converted to the same scale for comparison. 
