...
The sample consisted of 1044 students (56.6\% female, Mean= 16.71 years, 
sd=1.19, median=17 years). Age was measured as a categorical variable with 
(minimum=15, maximum=22); n=10 individuals between 20 and 22 were added to 
the category of 19+ years. The student performance dataset was downloaded 
from the UCI-MLR and saved as a data frame object in a python interactive
notebook. The data was collected from two secondary schools in the Alentejo
region of Portugal during the 2005-2006 school year and contained information
from a questionnaire and school reports of student grades \cite{cortez09}. 
The dataset included thirty predictor variables consisting of demographic 
...
(n=649). The target outcome was the final course grade. A binary binary 
measure of student performance was calculated based on final exam grades 
(Pass$>$10, Fail$<=$10) for purposes of comparison.
...
new observations. The sample dataset was divided using a 70 to 30 percent 
split to create the training set ($n_1$=731) and testing set ($n_2$=313). 
The following models were constructed on the training data and evaluated on 
the test set using R: linear regression, ridge regression, lasso regression, 
regression trees, random forests regression, gradient boosted regression. 
...
according ). The was no significant difference in the proportion of males 
and females who passed or failed (Chi-squared test, p=0.20). Student 
performance did vary however according to past course failures. The 
proportion of students who received a passing final grade was significantly 
different for those with no previous course failures compared to those with 
one or more previous failures (Chi-squared test, p<0.001). 

The proportion of students who received a passing grade was significantly 
different for students with plans to pursue higher education than 
students with no plans for higher education (Chi-squared test, p<0.001).

The proportion of pass/fail for students who studied for less than 2 hours 
per week was also significantly different than students who studied for 
2 hours or more per week (Chi-squared test, p<0.001). 

and with parents living together in the same 
household (88%).
...
the rate of Oxycodone use (e.g., `Oxycontin`) for each age group, and was 
...
Before conducting regression analyses, correlations among the variables were 
examined. The three course grade variables (G1, G2, G3) were significantly 
positively intercorrelated (all \textit{p}s<.001). The first and second course 
grade variables were excluded from subsequent analyses to control for 
multicolinearity. A multivariate linear regression of student performance 
including 28 independent variables (Table 1) was conducted on the training set.
The regression model was rerun excluding non-significant predictors, an anova 
revealed that there was no significant difference between the two models and 
the simpler model was retained as more parsimonious. Table X provides the 
coefficient estimates, standard error, t-values, F-values, degrees of freedom,
R-squared, and adjusted R-squared for the training set and testing set. The 
final regression model demonstrated the significant relationship between
student performance and the independent variables on both the training and 
test sets (both ps$<$0.01). Overall, between 25.5\% to 23.6\% of the variation 
in the predicted value of student performance was due to changes in the 
independent variables, taking into account the number of independent variables 
(adjusted $R^2$). On the training set, 12 out of the 28 predictor variables 
were significant, but only 7 of the 12 predictors in the testing set were 
significant. This difference in variability accounted for may suggest 
overfitting to the training set, but the difference in adjusted R-squared 
between the two models may be due to the smaller sample size of the test set. 
